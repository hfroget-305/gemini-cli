...
> = ({ messages, maxHeight, width, hasFocus }) => {
  ...
  // The widest prefix should be used to determine the box width
  const iconBoxWidth = isScreenReaderEnabled
    ? Math.max(
        stringWidth(SCREEN_READER_INFO),
        stringWidth(SCREEN_READER_WARNING),
        stringWidth(SCREEN_READER_ERROR),
        stringWidth(SCREEN_READER_DEBUG),
      )
    : 3;
